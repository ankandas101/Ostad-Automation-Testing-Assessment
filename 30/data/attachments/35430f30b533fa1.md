...
```
TypeError: productsPriceWithSine.split is not a function
```
...
```ts
  ...
  16 | 
  17 |         
> 18 |         const price = productsPriceWithSine.split('$');
     |                                             ^ TypeError: productsPriceWithSine.split is not a function
  19 |                 console.log('All Cart Products Price:', price);
  20 | 
  21 |         return price[1];
  22 |     }
  23 | 
  24 | 
  25 |     async goToCheckout() {
  26 |         await this.buttonLocator('checkout').click();
  27 | 
  28 |     }
  29 | 
  30 | 
